...
public class Driver
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
         ...
         for (Ship curr : myShips)
         {
              if (curr.getMax() > 100)
              {
		  //changed a deep copy to a shallow copy, so ships keep their ID
                   bigShips.add(curr);
              }
         }
	 ...
	 for(Cargo units : myPorts.get(0).getOutbound())
	     {
		 //Create new cargo with a deep copy to assign a new ID, keeping dest and tonnage
		 Cargo temp = new Cargo (units.getDest(), units.getTonnage());
		 newPort.addOutbound(temp);
	     }
