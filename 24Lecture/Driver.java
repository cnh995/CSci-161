...
public class Driver
{
    ...
    public static void main(String[] args)
    {
         ...
         catch (FileNotFoundException e)
         {
              ...
              System.err.println("The Port.txt file is not in the current directory!  Exiting Program!");
         }
...
//                   myPorters.add(new Transporter(parts[1], Double.parseDouble(parts[2]), Integer.parseInt(parts[3]), Integer.parseInt(parts[4])));
         ...
         while (!allCargoDelivered(myPorts, myPorters))
         {
              ...
              for (Transporter t : myPorters)
              {
                   ...
                   {
                        System.err.println(e.getMessage());
                        if (!(t instanceof Repairable))
                        {
                             if (!t.replaced())
                             {
                                  //clone transporter...Make sure this doesn't trigger more than once.
                                  Transporter clone = null;
                                  try
                                  {
                                       clone = t.cloneMe();
                                  }
                                  catch (BadDataException e0)
                                  {
                   ...
                   if (t.getDistance() == 0)
                   {
                        for (int p = 0; p < myPorts.size(); p++)
                        {
                             if (myPorts.get(p).getName().equals(t.getDest()) && t.getDistance() == 0)
                             {
                                  ...
                                  {
                                       System.err.println("This Exception was triggered during an Unload.");
                                       System.err.println(e.getMessage());
                                  }
                                  ...
                                  {
                                       System.err.println("This Exception was triggered during a Load.");
                                       System.err.println(e.getMessage());
                                  }
                             }
                        }
