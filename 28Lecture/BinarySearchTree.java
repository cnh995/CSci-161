...
{    
  ...
  public BinarySearchTree(int[] array, boolean optimize)
  {
      int median;
      ...
      //Your code for processing the array and filling up the BST goes here!
      if(optimize == true)
	  {      
	      Collections.sort(tempArray);
	      tempArray = recursionSort(tempArray);
    ...
    public ArrayList<Integer> recursionSort(ArrayList<Integer> array)
    {
	int medianIndex = array.size() / 2;
	ArrayList<Integer> lowerArray = new ArrayList<Integer>();
	ArrayList<Integer> upperArray = new ArrayList<Integer>();
	
	for(int i = 0; i < medianIndex; i++)
	    {
		lowerArray.add(array.get(i));
	    }
	
	for(int i = medianIndex + 1; i < array.size(); i++)
	    {
		upperArray.add(array.get(i));
	    }

	ArrayList<Integer> returnList = new ArrayList<Integer>();

	returnList.add(array.get(medianIndex));

	if(lowerArray.size() > 0)
	    {
		returnList.addAll(recursionSort(lowerArray));
	    }
	if(upperArray.size() > 0)
	    {
		returnList.addAll(recursionSort(upperArray));
	    }
 ...
 /**
  The method findHeight determines the height of the binary search tree.
  @param bstree The binary tree to look in.
  @return an integer height value
 */    
    private int findHeight(Node bstree)
    {
     // No Node
     if (bstree == null)
      return -1;
